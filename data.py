...
def load_and_process_data(file_path):
    ...
    df = df[df['Referencia Sem'].isin(['N-01'] + ['N0'] + [f'N{str(i).zfill(2)}' for i in range(-1, 13)])]
    return df
...
def get_item_data(df, selected_item):
    if selected_item == 'Todos':
        ...
        grouped_data = df.groupby(['Num_Sem', 'Semana'])['Total'].sum().reset_index()
        grouped_data['ITEM'] = 'Todos'
        grouped_data['SKU_DESCR'] = 'Todos los items'
        
        # Agrupar por semana y seleccionar el primer valor único de Venta_Real por producto
        venta_real_unica = df.groupby(['Semana', 'ITEM'])['Venta_Real'].first().reset_index()
        
        # Agrupar las ventas reales por semana sumando para todos los productos
        venta_real_total = venta_real_unica.groupby('Semana')['Venta_Real'].sum().reset_index()
        
        # Hacer un merge de los datos agrupados con las ventas reales totales por semana
        grouped_data = grouped_data.merge(venta_real_total, on='Semana', how='left')
        
        return grouped_data
    else:
        # Devolver los datos filtrados por item seleccionado
        return df[df['ITEM'] == selected_item]
...
def get_forecast_data(item_data, target_week):
    return item_data[item_data['Semana'] == target_week].sort_values('Num_Sem')